...
class AlterarExercicioFragment: DialogFragment() {

    lateinit var listener: AlterarExercicioFragmentListener
    lateinit var dialogView: View

    //define interface to communicate with activity
    interface AlterarExercicioFragmentListener {
        fun onEditExercicio(dialog: DialogFragment, exercicio: Exercicio)
        fun onDeleteExercicio(dialog: DialogFragment, id: Int)
    }
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        return activity?.let {
            ...
            dialogView.findViewById<Button>(R.id.fragmentAdicionarExercicioAddBtn).visibility = View.INVISIBLE
            dialogView.findViewById<Button>(R.id.fragmentAdicionarExercicioEditBtn).visibility = View.VISIBLE
            dialogView.findViewById<Button>(R.id.fragmentAdicionarExercicioRmBtn).visibility = View.VISIBLE
            dialogView.findViewById<EditText>(R.id.fragmentAdicionarExercicioPeso).setText(requireArguments().getFloat("peso").toString())
            dialogView.findViewById<EditText>(R.id.fragmentAdicionarExercicioSeries).setText(requireArguments().getInt("series").toString())
            dialogView.findViewById<EditText>(R.id.fragmentAdicionarExercicioRepeticoes).setText(requireArguments().getInt("repeticoes").toString())
            ...
            call.enqueue(object : Callback<ExercicioOutput> {
                override fun onResponse(call: Call<ExercicioOutput>, response: Response<ExercicioOutput>) {
                    ...
                    if(response.isSuccessful) {
                        ...
                        dialogView.findViewById<Button>(R.id.fragmentAdicionarExercicioEditBtn).setOnClickListener {
                            ...
                            when {
                                ...
                                series == "" -> {
                                    ...
                                    listener.onEditExercicio(this@AlterarExercicioFragment, response.body()!!.exercicio!!)
                                }
                            }
                        }

                        dialogView.findViewById<Button>(R.id.fragmentAdicionarExercicioRmBtn).setOnClickListener {

                            listener.onDeleteExercicio(this@AlterarExercicioFragment, id)
                        }
                    } else {
    ...
    override fun onAttach(context: Context) {
        ...
        try {
            listener = context as AlterarExercicioFragmentListener
        } catch (e: ClassCastException) {
            throw ClassCastException((context.toString() + " must implement AlterarExercicioFragmentListener"))
        }
    }
}